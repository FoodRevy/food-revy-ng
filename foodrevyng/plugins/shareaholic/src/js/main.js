(function ($) {
	...
	Shareaholic.bind_button_clicks = function (click_object, off) {
		...
		$(click_object.selector)
			.off('click.app_settings')
			.on('click.app_settings', function (e) {
				button = this;
				e.preventDefault();
				url = click_object.url(this);
				if (
					click_object.selector == '#general_settings' ||
					click_object.selector == '#affiliate_settings'
				) {
					window.open(url);
					return false;
				} else {
	...
	Shareaholic.click_objects = {
		app_settings: {
			...
			callback: function (button) {
				...
				app = $(button).data('app');
				text =
					'You can also use this shortcode to place this {{app}} App anywhere.';
				html =
          ...
          <textarea id='shortcode' name='widget_div' onclick='select();' readonly='readonly'></textarea> \
				...
				$(html).appendTo($modal);
				$('#shortcode_description').text(
					text.replace(/{{app}}/, Shareaholic.titlecase(app)),
				);
	...
	$(document).ready(function () {
		...
		if (Shareaholic.click_objects['unverified_general_settings']) {
			Shareaholic.bind_button_clicks(
