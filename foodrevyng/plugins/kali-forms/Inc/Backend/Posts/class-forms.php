...
class Forms {
	...
	public function init_email_deliverability_confirmation() {
		if ( ! empty( $_GET ) && ! empty( $_GET['confirm_email_delivery'] ) &&
			...
			$notifications->add_notice(
				[
					...
					'message' => esc_html__(
						'Email delivery confirmed', 'kaliforms'
					),
				]
			);
		}
	...
	public function register_submenus() {
		add_submenu_page(
			...
			esc_html__( 'SMTP Settings', 'kaliforms' ), 'manage_options',
			'kaliforms-email-settings', new Email_Settings_Page()
		);
		...
		add_submenu_page(
			...
			esc_html__( 'Extensions', 'kaliforms' ), 'manage_options',
			'kaliforms-extensions', new Extensions_Page()
		);
	}
	...
	public function enqueue() {
		...
		if ( 'kaliforms_forms' === get_post_type() && 'post' === $screen->base ) {
			...
			wp_register_script(
				'kaliforms-vendor', KALIFORMS_URL . 'assets/backend/js/vendor.js',
				[], KALIFORMS_VERSION
			);
			wp_enqueue_script(
				'kaliforms-forms', KALIFORMS_URL . 'assets/backend/js/forms.js', [
				'wp-i18n',
				'underscore',
				'kaliforms-vendor',
			], KALIFORMS_VERSION, true
			);

			wp_localize_script(
				'kaliforms-forms', 'KaliFormsObject', ( new JS_Vars() )->content
			);

			wp_set_script_translations(
				'kaliforms-forms', 'kaliforms', KALIFORMS_BASE . 'languages'
			);

			wp_enqueue_style(
				'kaliforms', KALIFORMS_URL . 'assets/backend/css/forms.css', [
				'kaliforms-media-view',
	...
	public function create_post_type() {
		...
		register_post_meta(
			'kaliforms_forms', 'kaliforms_field_components', [
				                 'single'       => true,
				                 'show_in_rest' => true,
		...
		register_post_meta(
			'kaliforms_forms', 'kaliforms_grid', [
				                 'single'       => true,
				                 'show_in_rest' => true,
				                 'description'  => 'A meta key associated with a string meta value.',
				                 'type'         => 'string',
		...
		add_filter(
			'image_size_names_choose', function ( $sizes ) {
	...
	public function register_metaboxes() {
		add_meta_box(
			'kaliforms_forms_builder', esc_html__( 'Form Builder', 'kaliforms' ),
			new Form_Builder(), 'kaliforms_forms', 'normal', 'high'
		);
	}
	...
	public function add_duplicate_link( $actions, $post ) {
		if ( $post->post_type === 'kaliforms_forms' ) {
			$actions['kaliforms-duplicate'] = sprintf(
				...
				$post->ID, $post->ID, esc_html__( 'Duplicate form', 'kaliforms' )
			);
		}
	...
	public function register_shortcode( $args ) {
		...
		$form = apply_filters(
			$this->slug . '_form_shortcode_finished_rendering', $form
		);
